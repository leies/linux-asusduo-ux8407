...
Do **not** DPMS-off the OLED (that stayed black). The lid helper only locks and dims, then logind sleeps. The HID keyboard backlight turns **off for s2idle and back on wake** (hook in `/usr/lib/systemd/system-sleep/`, confirmed on this machine).
